            ...
            <div class="chat__box box">
                ...
                <div id="chat_active" class="flex hidden h-full flex-col">
                    ...
                    <div class="flex items-center border-t border-slate-200/60 pb-10 pt-4 dark:border-darkmode-400 sm:py-4">
                        ...
                        <a class="cursor-pointer mr-5 flex h-8 w-8 flex-none items-center justify-center rounded-full bg-primary text-white sm:h-10 sm:w-10">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" data-lucide="send" class="send_btn lucide lucide-send stroke-1.5 h-4 w-4">
    ...
    <script>
        $(document).ready(function() {
            ...
            function dataUser() {
                $.ajax({
                    type: "post",
                    url: "<?= site_url('admin/chat/GetAllOrang') ?>",
                    dataType: "json",
                    success: function(r) {
                        var html = "";
                        var d = r.data;
						...
						<div data-id=${d.id_user} class="coba intro-x cursor-pointer box relative flex items-center p-5 mb-5">
                                ...
                                <div class="ml-2 overflow-hidden">
                                    <div class="flex items-center">
                                        ...
                                        <div class="text-xs text-gray-500 ml-2">${d.waktu}</div>
                                    </div>
            ...
            $('body').on('click', '.coba', function() {
                ...
                window.location.replace("<?= site_url('admin/chat/') ?>" + id);
            ...
            function dataChat() {
                ...
                $.ajax({
                    type: "post",
                    url: "<?= site_url('admin/chat/loadChat') ?>",
                    data: {
                        id_user: id_user,
                        id_balas: id_balas,
                    },
			...
			if (pesan != "") {
				$.ajax({
					type: "post",
                    url: "<?= site_url('admin/chat/KirimPesan') ?>",
					data: {
						id_user,
						id_balas,
						pesan
